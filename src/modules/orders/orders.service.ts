...
export class OrdersService {
  ...
  async onModuleInit() {
    const existingOrderBook: OrderBookDto = await this.redisGateway.get(ORDER_BOOK_KEY);

    if (existingOrderBook.buyOrders.length === 0 && existingOrderBook.sellOrders.length === 0) {
  ...
  async placeOrder( orderId: number, amount: number,price: number,direction: string) {
  ...
  private async handleSellOrder( orderId: number, amount: number, price: number,orderBook: OrderBookDto,) {
    ...
    for (let i = 0; i < orderBook.buyOrders.length; i++) {
      ...
      if (buyObj.direction === OrderDirection.BUY && buyObj.price >= price) {
        ...
        return await this.processSellOrderMatch(orderId, amount,price,buyObj,orderBook); //match 
      }
    }
  ...
  private async processSellOrderMatch(orderId: number,amount: number,  price: number, buyObj: Order, orderBook: OrderBookDto,) {
    ...
    if (amount > buyObj.amount) { // ako 10 > 9
      amount = amount - buyObj.amount;    // 10 - 9 =1

      orderBook.buyOrders = orderBook.buyOrders.filter(order => order.id !== buyObj.id);//трие от редис
      await this.orderRepository.delete(buyObj.id); // трие в базата
      
      const newOrder = await this.createNewOrder(amount, price, OrderDirection.SELL);
      ...
      if (buyObj.amount <= 0) {
        ...
        orderBook.buyOrders = orderBook.buyOrders.filter( (order ) => order.id !== buyObj.id);// Remove matched order
  ...
  private async createNewSellOrder( amount: number, price: number, orderBook: OrderBookDto) {
    const newOrder = await this.createNewOrder(amount,price,OrderDirection.SELL);
  ...
  private async createNewOrder(amount: number, price: number, direction: OrderDirection) {
  ...
  private async handleBuyOrder(orderId: number,amount: number,price: number, orderBook: OrderBookDto) {
    ...
    for (let i = 0; i < orderBook.sellOrders.length; i++) {
      ...
      if (sellObj.direction === OrderDirection.SELL && sellObj.amount >= amount && sellObj.price <= price) {
        idOrderMatched = true;
        return await this.processBuyOrderMatch(orderId,amount,price,sellObj, orderBook);
      }
    }
  ...
  private async processBuyOrderMatch(orderId: number,amount: number,price: number,sellObj: Order,orderBook: OrderBookDto) {
    ...
    if (sellObj.amount <= 0) {
      ...
      orderBook.sellOrders = orderBook.sellOrders.filter( (order) => order.id !== sellObj.id); 
    }
  ...
  private async createNewBuyOrder( amount: number, price: number, orderBook: OrderBookDto) {
    const newOrder = await this.createNewOrder(amount,price, OrderDirection.BUY);
 ...
 private async handleDublicateBuyOrders(orderBook: OrderBookDto, price: number) {
  ...
  //ако има едни и същи хора с еднакъв order просто да се увеличи amount-a, не се създава нов запис
  private async handleDublicateSellOrders(orderBook: OrderBookDto, price: number) {
    const existingOrder = orderBook.sellOrders.find((order) =>

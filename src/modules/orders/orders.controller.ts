...
export class OrdersController {
  ...
  @Get('id/:id')
  async findSpecificOrder(@Param('id', ParseIntPipe) id: number): Promise<Order> {
    return await this.ordersService.findOrder(id);
  }

  @Get(':direction')
  async getOrdersByDirectionAndStatus( @Param('direction') direction: string,@Query('status') status: string): Promise<Order[]> {
